...
public abstract class DButils {
	...
	public static double getRemainingTimeOfParkingSpot(String PSname, LocalDateTime simStartTime,
			LocalDateTime simEndTime) {
		ResultSet rs;
	...
	public static void _spacial_printAfekaPLAnalisys() {
		ResultSet rs = Controller.DBcon.get_AfekaPLData_Data();
		PrintWriter writer = null;
		try {
			writer = new PrintWriter("Afeka_data_extraction.txt", "UTF-8");
		...
		try {
			...
			while (rs.next()) {
				...
				Date datetime = rs.getTimestamp(1);
				int licensePlate = rs.getInt(2);
				String reason = rs.getString(3);
				
				if (reason.equals("Standard entrance")) {
					if (rs.next()) {
						LocalDateTime nextDatetime = rs.getTimestamp(1).toLocalDateTime();
						int nextLicensePlate = rs.getInt(2);
						String nextReason = rs.getString(3);
						
						if (nextReason.equals("Standard exit") && licensePlate == nextLicensePlate) {
							// write to file
							//writer.println(datetime + ", " + licensePlate + ", " + calcDifference(datetime, nextDatetime));						

							//System.out.println("start: " + datetime + ", licensePlate: " + licensePlate + ", Duration: " + calcDifference(datetime, nextDatetime));
						}
						else {
							rs.previous();
						}
